ss_parse_args <- function(args = commandArgs(trailingOnly = TRUE)) {
  opts <- list(
    ...
    prompt_path = here::here(
      "classification", "prompts", "live_chat_logs", "chat_log_summarization_openai.md"
    ),
...
ss_print_help <- function() {
  cat(
    paste(
      "Usage:",
      "  Rscript scripts/run/Text_Replay_Analysis/Text_replay_analysis_openAI [options]",
      "",
      "Options:",
      ...
      "  --prompt-path PATH           Prompt markdown file with ---SYSTEM---/---USER---",
