...
source("r_scripts/lib/utils/talent_select.R")
source("r_scripts/lib/duckdb/db_connect.R")
source("r_scripts/lib/duckdb/db_schema.R")
source("r_scripts/lib/duckdb/ingest_videos.R")
source("r_scripts/lib/duckdb/pending.R")
source("r_scripts/lib/ChatGPT/chatgpt_load_all.R")
source("r_scripts/lib/stream_classification/talent_rules.R")
source("r_scripts/lib/stream_classification/prompt_builder.R")
...
classification_timeout_seconds <- suppressWarnings(
  as.integer(Sys.getenv("CLASSIFICATION_TIMEOUT_SECONDS", unset = "120"))
)
if (is.na(classification_timeout_seconds) || classification_timeout_seconds < 1) {
  classification_timeout_seconds <- 120L
}
...
message("OpenAI request timeout (seconds): ", classification_timeout_seconds)
...
classify_batch <- function(
  ...
  for (attempt in seq_len(attempts)) {
    parsed <- tryCatch(
      {
        message(
          "Sending classification batch to OpenAI (attempt ",
          attempt,
          "/",
          attempts,
          ", rows=",
