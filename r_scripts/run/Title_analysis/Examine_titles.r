...
list.files(here("r_scripts", "lib", "report_tables"), pattern = "[rR]$", full.names = TRUE) %>%
  walk(source)
list.files(here("r_scripts", "lib", "clean_data"), pattern = "[rR]$", full.names = TRUE) %>%
  keep(~ basename(.) != "CleanData.R") %>%
  walk(source)
list.files(here("r_scripts", "lib", "ENA_prep"), pattern = "[rR]$", full.names = TRUE) %>%
  walk(source)
list.files(here("r_scripts", "lib", "plots"), pattern = "\\.[rR]$", full.names = TRUE, recursive = TRUE) %>%
  walk(source)
...
# Performance bins (shared z-score helper in r_scripts/lib/clean_data/analytics_core.R)
min_n <- 10
