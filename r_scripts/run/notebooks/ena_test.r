...
source_dir <- function(..., pattern = "[rR]$") {
  list.files(
    here::here(...),
    pattern = pattern,
    full.names = TRUE
...
coding_book <- resolve_datalake_path(
  r"(Z:\DataLake\Sun_Data_Analytics\Processed\Talent_Data\Qualitative Codebook\current\personality_qualitative_code_log.csv)"
)

batch_run_dirs <- c(
  r"(Z:\DataLake\Sun_Data_Analytics\Processed\Talent_Data\qualitative_batch_runs\Nova_Zcpg453BNU_full_expanded_codes_2026-05-20)",
  r"(Z:\DataLake\Sun_Data_Analytics\Processed\Talent_Data\qualitative_batch_runs\Ava_k84ImiUcjbE_full_expanded_codes_2026-05-20)"
)
