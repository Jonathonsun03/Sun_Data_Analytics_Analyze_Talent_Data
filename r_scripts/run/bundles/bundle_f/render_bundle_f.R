...
input_rmd <- rr_arg_value(args, "--input", repo_path("r_scripts", "notebooks", "reports", "bundle_f", "bundle_f.qmd"))
...
output_prefix <- rr_arg_value(args, "--output-prefix", "Bundle_F")
...
cat("Bundle F render targets:", paste(talents, collapse = ", "), "\n")
cat("Input document:", input_rmd, "\n")
if (output_dir_supplied) {
  cat("Output dir:", output_dir, "\n")
} else {
  cat("Output routing: talent-specific Bundle F folders under ", output_dir, "\n", sep = "")
}
...
result_df <- rr_render_for_talents(
  ...
  output_dir = if (output_dir_supplied) {
    ...
    function(talent, slug) {
      normalizePath(
        file.path(output_dir, talent, "reports", "bundle_f"),
        winslash = "/",
        mustWork = FALSE
      )
    }
