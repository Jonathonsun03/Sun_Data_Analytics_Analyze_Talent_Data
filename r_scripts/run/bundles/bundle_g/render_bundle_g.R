...
parse_content_types <- function(args) {
  content_csv <- rr_arg_value(args, "--content-types", "")
  content_one <- rr_arg_value(args, "--content-type", "")
  values <- c(rr_split_csv_values(content_csv), rr_split_csv_values(content_one))
  values <- tolower(trimws(values))
  values <- values[nzchar(values)]
  values <- gsub("^videos$", "video", values)
  values <- gsub("^shorts$", "short", values)
  if (length(values) == 0 || "all" %in% values) {
...
input_rmd <- rr_arg_value(args, "--input", repo_path("r_scripts", "notebooks", "reports", "bundle_g", "bundle_g.qmd"))
input_rmd <- resolve_repo_or_abs(input_rmd)
if (!file.exists(input_rmd)) {
  stop("Input document not found: ", input_rmd)
}
...
output_prefix <- rr_arg_value(args, "--output-prefix", "Bundle_G")
...
reference_day <- trimws(rr_arg_value(args, "--reference-day", "Monday"))
content_types <- parse_content_types(args)
...
if (!is.na(start_date) || !is.na(end_date)) {
  ...
  end_out <- as.character(Sys.Date())
  start_out <- as.character(Sys.Date() - as.integer(window_days) + 1L)
} else {
  start_out <- NA_character_
  end_out <- NA_character_
}

included_date_range <- if (!is.na(start_out) || !is.na(end_out)) {
...
cat("Bundle G render targets:", paste(talents, collapse = ", "), "\n")
cat("Input document:", input_rmd, "\n")
if (output_dir_supplied) {
  cat("Output dir:", output_dir, "\n")
} else {
  cat("Output routing: talent-specific Bundle G folders under ", output_dir, "\n", sep = "")
}
cat("Data source:", data_source, "\n")
cat("Data root:", data_root, "\n")
cat("Content types:", paste(content_types, collapse = ", "), "\n")
cat("Reference day:", reference_day, "\n")
...
result_df <- rr_render_for_talents(
  ...
  output_dir = if (output_dir_supplied) {
    ...
    function(talent, slug) {
      normalizePath(
        file.path(output_dir, talent, "reports", "bundle_g"),
        winslash = "/",
        mustWork = FALSE
      )
    }
  ...
  params_builder = function(talent, render_dir) {
    list(
      ...
      start_date = if (is.na(start_out)) NULL else start_out,
      end_date = if (is.na(end_out)) NULL else end_out,
