...
input_rmd <- rr_arg_value(args, "--input", repo_path("r_scripts", "notebooks", "reports", "Bundle_A", "Bundle_A.Rmd"))
...
data_source <- tolower(trimws(rr_arg_value(args, "--data-source", "staging")))
...
if (output_dir_supplied) {
  ...
  cat("Output routing: talent-specific Bundle A folders under ", output_dir, "\n", sep = "")
}
...
result_df <- rr_render_for_talents(
  ...
  output_dir = if (output_dir_supplied) {
    ...
    function(talent, slug) {
      normalizePath(
        file.path(output_dir, talent, "reports", "bundle_A"),
        winslash = "/",
        mustWork = FALSE
      )
    }
