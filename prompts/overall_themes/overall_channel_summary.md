...
Execution guard:
- If you are reading this prompt inside a `codex exec` session that was launched by `bin/linux/codex_prompts/overall_themes/overall_channel_summary.sh`, do not invoke `bin/linux/codex_prompts/overall_themes/overall_channel_summary.sh` again.
...
- If scripting is useful for CSV or markdown inspection, use `python3`, not `python`.
