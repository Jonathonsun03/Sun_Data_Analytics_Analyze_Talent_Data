...
- The canonical runner for this workflow is `scripts/run/stream_summaries/personality/personality_profile_v3_incremental_open_coding.py`.
...
- Keep workflow-specific code inside `scripts/run/stream_summaries/personality/` unless logic is truly reusable across workflows, in which case move only the shared helper pieces into `scripts/lib/stream_summaries/`.
