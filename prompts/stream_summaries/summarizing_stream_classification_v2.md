...
- The canonical runner for this workflow is `scripts/run/stream_summaries/summary_classification/summary_classification_incremental.py`.
- Keep workflow-specific code inside `scripts/run/stream_summaries/summary_classification/` unless logic is truly reusable across workflows, in which case move only the shared helper pieces into `scripts/lib/stream_summaries/`.
