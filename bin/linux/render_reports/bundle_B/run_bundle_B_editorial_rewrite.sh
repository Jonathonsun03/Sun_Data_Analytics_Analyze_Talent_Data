...
PROMPT_SPEC="prompts/reports/bundle_b/07_editorial_review/02_full_report_editorial_rewrite/prompt.md"
...
usage() {
  cat <<'USAGE'
Usage:
  bin/linux/render_reports/bundle_B/run_bundle_B_editorial_rewrite.sh [options]

Description:
  Rewrites all client-facing Bundle B interpretation paragraphs so the final
...
resolve_logs_root() {
  ...
  printf '%s' "${analytics_root}/Processed/Logs/codex_prompts/reports/bundle_b_editorial_rewrite"
}
...
echo "[bundle-b-editorial] Started: $(date -u +"%Y-%m-%dT%H:%M:%SZ")"
echo "[bundle-b-editorial] Prompt spec: ${PROMPT_SPEC_PATH}"
echo "[bundle-b-editorial] Datalake root: ${DATALAKE_ROOT}"
echo "[bundle-b-editorial] Log root: ${LOG_ROOT}"
echo "[bundle-b-editorial] Talents: ${TALENTS[*]}"
...
for talent_query in "${TALENTS[@]}"; do
  ...
  if [[ ${rc} -ne 0 ]]; then
    status=1
    echo "[bundle-b-editorial] Error: ${talent_folder_name}" >&2
    continue
  fi

  interpret_root="${DATALAKE_ROOT}/${talent_folder_name}/${REPORT_SUBDIR}/${BUNDLE_NAME}/${INTERPRET_DIR}"
  work_dir="${interpret_root}/07_editorial_review/02_full_report_editorial_rewrite"
  input_file="${work_dir}/input.md"
  current_json="${work_dir}/current_paragraphs.json"
  rewrite_json="${work_dir}/rewrite_output.json"
  review_note_file="${interpret_root}/07_editorial_review/01_full_report_editorial_review/output.md"
  talent_log_dir="${LOG_ROOT}/$(safe_slug "${talent_folder_name}")"
  mkdir -p "${work_dir}" "${talent_log_dir}"

  if [[ ! -d "${interpret_root}" ]]; then
    status=1
    echo "[bundle-b-editorial] Missing interpretation root: ${interpret_root}" >&2
    continue
  fi

  report_files=()
  exec_summary="${interpret_root}/06_report_bookends/01_executive_summary/output.md"
  conclusion="${interpret_root}/06_report_bookends/02_conclusion/output.md"
  [[ -f "${exec_summary}" ]] && report_files+=("${exec_summary}")
  while IFS= read -r f; do
    [[ -z "${f}" ]] && continue
    report_files+=("${f}")
  done < <(find \
    "${interpret_root}/01_format_level_retention_consistency" \
    "${interpret_root}/02_category_strengths_and_weaknesses" \
    "${interpret_root}/03_content_attributes_matter_for_engagement_outcomes" \
    "${interpret_root}/04_scheduling_effects_matter_for_publishing_decisions" \
    "${interpret_root}/05_turn_findings_into_action" \
    -name 'output.md' 2>/dev/null | sort)
  ...
  if [[ ${#report_files[@]} -eq 0 ]]; then
    status=1
    echo "[bundle-b-editorial] No report-facing interpretation outputs matched for talent: ${talent_query}" >&2
    continue
  fi
...
PY
  ...
  {
    echo "# Bundle B Editorial Rewrite Task"
    echo
    echo "Rewrite the full set of client-facing Bundle B paragraphs so the report reads like one cohesive, professionally edited narrative."
    echo
    echo "## Output rules"
  ...
  echo
  echo "[bundle-b-editorial] Talent: ${talent_query}"
  echo "[bundle-b-editorial] Interpretation root: ${interpret_root}"
  echo "[bundle-b-editorial] Rewrite input: ${input_file}"
  echo "[bundle-b-editorial] Rewrite output JSON: ${rewrite_json}"
  echo "[bundle-b-editorial] Log file: ${log_file}"
  ...
  if [[ ${rc_rewrite} -ne 0 || ! -s "${rewrite_json}" ]]; then
    status=1
    echo "[bundle-b-editorial] Editorial rewrite failed for talent: ${talent_folder_name}" >&2
    echo "[bundle-b-editorial] See log: ${log_file}" >&2
    continue
  fi
...
PY
  then
    status=1
    echo "[bundle-b-editorial] Rewrite JSON validation failed for talent: ${talent_folder_name}" >&2
    echo "[bundle-b-editorial] See log: ${log_file}" >&2
    continue
  fi

  echo "[bundle-b-editorial] Overwrote ${#report_files[@]} report paragraph(s) for ${talent_folder_name}."
done
