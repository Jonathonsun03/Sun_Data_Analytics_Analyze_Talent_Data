...
DEFAULT_IMPORT_SCRIPT="r_scripts/run/bundles/bundle_B/import_data.r"
...
usage() {
  cat <<'USAGE'
Usage:
  bin/linux/render_reports/bundle_B/run_bundle_B_artifacts.sh [options]

Description:
  Builds Bundle B evidence artifacts for one or more talents and writes them to:
    <datalake_root>/<talent>/reports/bundle_B/artifacts/

  It calls:
    r_scripts/run/bundles/bundle_B/import_data.r
...
echo "[bundle-b-artifacts] Started: $(date -u +"%Y-%m-%dT%H:%M:%SZ")"
echo "[bundle-b-artifacts] Repo root: ${REPO_ROOT}"
echo "[bundle-b-artifacts] Import script: ${IMPORT_SCRIPT_PATH}"
echo "[bundle-b-artifacts] Input source: ${INPUT_SOURCE}"
echo "[bundle-b-artifacts] Talents: ${TALENTS[*]}"
...
for talent_query in "${TALENTS[@]}"; do
  ...
  echo
  echo "[bundle-b-artifacts] Talent: ${talent_query}"
  echo "[bundle-b-artifacts] Command: ${cmd[*]}"
  ...
  if [[ ${rc} -ne 0 ]]; then
    status=1
    echo "[bundle-b-artifacts] Failed for talent: ${talent_query}" >&2
  fi
done
