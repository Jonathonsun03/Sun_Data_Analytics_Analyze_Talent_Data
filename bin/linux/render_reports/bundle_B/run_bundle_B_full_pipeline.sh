...
ARTIFACTS_RUNNER="bin/linux/render_reports/bundle_B/run_bundle_B_artifacts.sh"
INTERPRET_RUNNER="bin/linux/render_reports/bundle_B/run_bundle_B_interpretations.sh"
EDITORIAL_RUNNER="bin/linux/render_reports/bundle_B/run_bundle_B_editorial_rewrite.sh"
REPORT_RUNNER="bin/linux/render_reports/bundle_B/run_bundle_B_render_only.sh"
...
usage() {
  cat <<'USAGE'
Usage:
  bin/linux/render_reports/bundle_B/run_bundle_B_full_pipeline.sh [options]

Description:
  Runs the current Bundle B pipeline in four stages:
    1) build figures/tables/artifacts
    2) run interpretation generation
    3) run editorial rewrite over the generated report text
    4) render the final Bundle B report
...
run_stage() {
  ...
  shift
  echo
  echo "[bundle-b-full] ${label}"
  echo "[bundle-b-full] Command: $*"
  "$@"
}

echo "[bundle-b-full] Started: $(date -u +"%Y-%m-%dT%H:%M:%SZ")"
echo "[bundle-b-full] Repo root: ${REPO_ROOT}"

status=0

set +e
run_stage "Stage 1/4: Build artifacts" "${ARTIFACTS_RUNNER}" "${PASS_ARGS[@]}"
rc_artifacts=$?
if [[ ${rc_artifacts} -ne 0 ]]; then
  status=1
  echo "[bundle-b-full] Artifact stage failed with status ${rc_artifacts}" >&2
fi
...
if [[ ${status} -eq 0 && "${SKIP_INTERPRETATION}" != "true" ]]; then
  ...
  if [[ ${rc_interp} -ne 0 ]]; then
    status=1
    echo "[bundle-b-full] Interpretation stage failed with status ${rc_interp}" >&2
  fi
elif [[ "${SKIP_INTERPRETATION}" == "true" ]]; then
  echo
  echo "[bundle-b-full] Stage 2/4 skipped: interpretation"
fi
...
if [[ ${status} -eq 0 && "${SKIP_EDITORIAL_REWRITE}" != "true" ]]; then
  ...
  if [[ ${rc_edit} -ne 0 ]]; then
    status=1
    echo "[bundle-b-full] Editorial rewrite stage failed with status ${rc_edit}" >&2
  fi
elif [[ "${SKIP_EDITORIAL_REWRITE}" == "true" ]]; then
  echo
  echo "[bundle-b-full] Stage 3/4 skipped: editorial rewrite"
fi
...
if [[ ${status} -eq 0 ]]; then
  ...
  if [[ ${rc_render} -ne 0 ]]; then
    status=1
    echo "[bundle-b-full] Render stage failed with status ${rc_render}" >&2
  fi
fi
set -e
...
echo
if [[ ${status} -eq 0 ]]; then
  echo "[bundle-b-full] Complete: success"
else
  echo "[bundle-b-full] Complete: one or more stages failed" >&2
fi
