...
ARTIFACTS_RUNNER="bin/linux/render_reports/bundle_G/run_bundle_G_artifacts.sh"
REPORT_RUNNER="bin/linux/render_reports/bundle_G/run_bundle_G_render_only.sh"
...
usage() {
  cat <<'USAGE'
Usage:
  bin/linux/render_reports/bundle_G/run_bundle_G_full_pipeline.sh [options]

Description:
  Runs the current Bundle G pipeline:
    1) compatibility artifact stage
    2) render and publish the final Bundle G topic-by-weekday report

Compatibility flags accepted and ignored until Bundle G interpretation wrappers exist:
  --skip-interpretation
...
run_stage() {
  ...
  shift
  echo
  echo "[bundle-g-full] ${label}"
  echo "[bundle-g-full] Command: $*"
  "$@"
}

echo "[bundle-g-full] Started: $(date -u +"%Y-%m-%dT%H:%M:%SZ")"
echo "[bundle-g-full] Repo root: ${REPO_ROOT}"
...
if [[ ${rc_artifacts} -ne 0 ]]; then
  status=1
  echo "[bundle-g-full] Artifact stage failed with status ${rc_artifacts}" >&2
fi
...
if [[ ${status} -eq 0 ]]; then
  ...
  if [[ ${rc_render} -ne 0 ]]; then
    status=1
    echo "[bundle-g-full] Render stage failed with status ${rc_render}" >&2
  fi
fi
set -e
...
echo
if [[ ${status} -eq 0 ]]; then
  echo "[bundle-g-full] Complete: success"
else
  echo "[bundle-g-full] Complete: one or more stages failed" >&2
fi
