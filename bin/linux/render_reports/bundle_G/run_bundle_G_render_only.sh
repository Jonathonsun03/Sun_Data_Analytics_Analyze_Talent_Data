...
RENDER_SCRIPT="r_scripts/run/bundles/bundle_g/render_bundle_g.R"
...
usage() {
  cat <<'USAGE'
Usage:
  bin/linux/render_reports/bundle_G/run_bundle_G_render_only.sh [options]

Description:
  Renders Bundle G topic-by-weekday reports and publishes final HTML to:
...
publish_rendered_report() {
  ...
  if [[ ! -f "${rendered_file}" ]]; then
    ...
    if [[ ${#matches[@]} -eq 1 && -f "${matches[0]}" ]]; then
      ...
      echo "Error: could not identify rendered Bundle G HTML under ${output_dir}" >&2
      return 1
    fi
  fi
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --end-date)
      ...
      ;;
    --content-type)
      [[ $# -ge 2 ]] || { echo "Error: --content-type requires a value" >&2; exit 1; }
      CONTENT_TYPES="$2"
      shift 2
      ;;
    --content-types)
      [[ $# -ge 2 ]] || { echo "Error: --content-types requires a value" >&2; exit 1; }
      CONTENT_TYPES="$2"
      shift 2
      ;;
    --reference-day)
      [[ $# -ge 2 ]] || { echo "Error: --reference-day requires a value" >&2; exit 1; }
      REFERENCE_DAY="$2"
      shift 2
      ;;
    --input-source)
...
for talent_query in "${TALENTS[@]}"; do
  ...
  talent="$(trim "${talent}")"
  output_dir="${DATALAKE_ROOT}/${talent}/reports/bundle_g"
  current_dir="${output_dir}/report/current"
  archive_dir="${output_dir}/report/archive"
  output_prefix_for_run="Bundle_G_${RUN_DATE}_${WINDOW_SEGMENT}"
  cmd=(
    "${RSCRIPT_BIN}"
    "${RENDER_SCRIPT_PATH}"
    --talent "${talent}"
