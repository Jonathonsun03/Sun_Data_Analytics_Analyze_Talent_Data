...
RUNNER_B="bin/linux/render_reports/bundle_B/run_bundle_B_render_only.sh"
...
usage() {
  cat <<'USAGE'
Usage:
  bin/linux/render_reports/run_monthly_bundle_B_render_only.sh [options] [-- extra wrapper args]

Description:
  Runs simplified Bundle B render-only reports for all talents across multiple
...
run_window() {
  ...
  echo
  echo "[monthly-bundle-b-render-only] Window: ${window_days} days"
  echo "[monthly-bundle-b-render-only] Command: $* --window-days ${window_days}"
  "$@" --window-days "${window_days}"
}

echo "[monthly-bundle-b-render-only] Started: $(date -u +"%Y-%m-%dT%H:%M:%SZ")"
echo "[monthly-bundle-b-render-only] Repo root: ${REPO_ROOT}"
echo "[monthly-bundle-b-render-only] Windows: ${WINDOWS[*]}"
echo "[monthly-bundle-b-render-only] Input source: ${INPUT_SOURCE}"
echo "[monthly-bundle-b-render-only] Include interpretations: ${INCLUDE_INTERPRETATIONS}"
echo "[monthly-bundle-b-render-only] LANG: ${LANG:-}"
echo "[monthly-bundle-b-render-only] LC_ALL: ${LC_ALL:-}"

status=0

set +e
for window_days in "${WINDOWS[@]}"; do
  run_window "${window_days}" "${RUNNER_B}" "${COMMON_ARGS[@]}" "${EXTRA_WRAPPER_ARGS[@]}"
  rc=$?
  if [[ ${rc} -ne 0 ]]; then
    status=1
    echo "[monthly-bundle-b-render-only] Window ${window_days} failed with status ${rc}" >&2
  fi
done
set -e
...
echo
if [[ ${status} -eq 0 ]]; then
  echo "[monthly-bundle-b-render-only] Complete: success"
else
  echo "[monthly-bundle-b-render-only] Complete: one or more windows failed" >&2
fi
