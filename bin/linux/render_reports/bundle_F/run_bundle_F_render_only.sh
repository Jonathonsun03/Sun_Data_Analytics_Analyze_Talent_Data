...
RENDER_SCRIPT="r_scripts/run/bundles/bundle_f/render_bundle_f.R"
...
usage() {
  cat <<'USAGE'
Usage:
  bin/linux/render_reports/bundle_F/run_bundle_F_render_only.sh [options]

Description:
  Renders Bundle F day-of-week reports and publishes final HTML to:
...
publish_rendered_report() {
  ...
  if [[ ! -f "${rendered_file}" ]]; then
    ...
    if [[ ${#matches[@]} -eq 1 && -f "${matches[0]}" ]]; then
      ...
      echo "Error: could not identify rendered Bundle F HTML under ${output_dir}" >&2
      return 1
    fi
  fi
...
for talent_query in "${TALENTS[@]}"; do
  ...
  talent="$(trim "${talent}")"
  output_dir="${DATALAKE_ROOT}/${talent}/reports/bundle_f"
  current_dir="${output_dir}/report/current"
  archive_dir="${output_dir}/report/archive"
  output_prefix_for_run="Bundle_F_${RUN_DATE}_${WINDOW_SEGMENT}"
  cmd=(
    "${RSCRIPT_BIN}"
    "${RENDER_SCRIPT_PATH}"
    --talent "${talent}"
