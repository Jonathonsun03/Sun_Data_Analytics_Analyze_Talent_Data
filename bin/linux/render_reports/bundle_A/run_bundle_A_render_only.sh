...
usage() {
  cat <<'USAGE'
Usage:
  bin/linux/render_reports/bundle_A/run_bundle_A_render_only.sh [options] [-- extra render args]
...
Examples:
  ...
  Batch from file:
    bin/linux/render_reports/bundle_A/run_bundle_A_render_only.sh --talents-file notes/talent_list.txt --window-days 30
...
publish_rendered_report() {
  ...
  if [[ ! -f "${rendered_file}" ]]; then
    ...
    if [[ ${#matches[@]} -eq 1 && -f "${matches[0]}" ]]; then
      ...
      echo "Error: could not identify rendered Bundle A HTML under ${output_dir}" >&2
      return 1
    fi
  fi
