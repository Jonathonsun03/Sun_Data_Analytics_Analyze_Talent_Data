...
Talent selection:
  ...
  --all                        Resolve all talents from staging root
...
load_talents_all() {
  ...
  out="$(
    cd "${REPO_ROOT}"
    "${RSCRIPT_BIN}" --vanilla -e "source('scripts/lib/utils/staging_root.R'); source('scripts/lib/utils/talent_select.R'); cat(list_talents()\$name, sep='\n')"
  )"
