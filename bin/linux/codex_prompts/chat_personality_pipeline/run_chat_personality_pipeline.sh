...
usage() {
  cat <<'EOF'
Usage:
  bin/linux/codex_prompts/chat_personality_pipeline/run_chat_personality_pipeline.sh [options]

Options:
  --talent NAME      Run the full chat personality pipeline for the exact talent folder NAME.
...
Pipeline order:
  1. chat_personality_open_coding.sh
  2. chat_shared_behavior_baseline.sh
  3. chat_personality_unique_features.sh
  4. chat_personality_qualitative_codebook.sh
  5. chat_personality_profile.sh
EOF
}
...
STEPS=(
  "scoped:$CODEX_PROMPTS_DIR/chat_personality/chat_personality_open_coding.sh"
  "global:$CODEX_PROMPTS_DIR/chat_personality/chat_shared_behavior_baseline.sh"
  "scoped:$CODEX_PROMPTS_DIR/chat_personality/chat_personality_unique_features.sh"
  "global:$CODEX_PROMPTS_DIR/chat_personality/chat_personality_qualitative_codebook.sh"
  "scoped:$CODEX_PROMPTS_DIR/chat_personality/chat_personality_profile.sh"
)
...
run_step() {
  ...
  if [[ -n "$TALENT_SCOPE" && "$step_scope" == "global" ]]; then
    step_args+=(--talent "$TALENT_SCOPE")
  fi
  ...
  echo
  echo "=== Running chat personality pipeline step: $step_name ==="
  if [[ "$DRY_RUN" -eq 1 ]]; then
    ...
    if ! "$step_path" "${step_args[@]}"; then
      echo "Error: chat personality pipeline step failed: $step_name" >&2
      exit 1
    fi
  fi
}

echo "Chat personality pipeline runner"
...
if [[ "$DRY_RUN" -eq 1 ]]; then
  ...
  echo "Chat personality pipeline complete."
fi
