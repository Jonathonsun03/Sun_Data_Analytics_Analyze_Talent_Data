...
usage() {
  cat <<'EOF'
Usage:
  bin/linux/codex_prompts/chat_personality/chat_personality_qualitative_codebook.sh [options] [prompt_file]
...
if [[ -n "$TALENT_SCOPE" || "$HAS_WINDOW_SCOPE" -eq 1 ]]; then
  ...
  cp "$PROMPT_FILE" "$RUN_PROMPT_FILE"
  {
    ...
    if [[ -n "$TALENT_SCOPE" ]]; then
      ...
      echo "- Use this as a focus note while preserving cross-talent codebook continuity."
    fi
