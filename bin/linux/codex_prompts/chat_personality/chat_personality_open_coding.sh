...
DEFAULT_PROMPT_FILE="$REPO/prompts/chat_personality/chat_personality_open_coding.md"
...
LOG_DIR="$LOG_ROOT/chat_personality/chat_personality_open_coding"
RUN_TS="$(date +%Y-%m-%d_%H-%M-%S)"
RUN_SLUG="chat_personality_open_coding"
...
usage() {
  cat <<'EOF'
Usage:
  bin/linux/codex_prompts/chat_personality/chat_personality_open_coding.sh [options] [prompt_file]
  ...
  if [[ "$HAS_WINDOW_SCOPE" -eq 1 ]]; then
    {
      ...
      echo "- Focus new analysis on chat rows, source files, stream dates, upload dates, or source metadata inside this window."
      echo "- Use older existing outputs only as continuity, cumulative state, and comparison context."
