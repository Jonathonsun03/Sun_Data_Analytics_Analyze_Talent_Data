...
- Keep prompt specs in `prompts/`, organized to mirror `bin/linux/codex_prompts/` (`summaries/`, `monetary/`, `streamer_personality/`, and `shared_qualities/`).

Current subareas:
- `summary_classification/` for summary-driven qualitative coding.
- `monetary_analysis/` for raw-text money-event analysis.
- `streamer_personality/` for personality profiling and synthesis workflows.
